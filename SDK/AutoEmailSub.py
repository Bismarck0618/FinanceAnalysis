...
def sendMail(subject, recipient, text, *attachmentFilePaths):
    ...
    '''
    senderInfo = loadPickle(os.path.basename('./Email_Setting'), "senderInfo.txt")
    ...
    smpt_addr = senderInfo.split("?")[3]                # 邮件服务器地址
    ...
    # try:
    mailServer = smtplib.SMTP(smpt_addr)                            # 连接腾讯邮件的服务器；SMTP（Simple Mail Transfer Protocol）即简单邮件传输协议，用于由源地址到目的地址传送邮件的规则，由它来控制信件的中转方式
